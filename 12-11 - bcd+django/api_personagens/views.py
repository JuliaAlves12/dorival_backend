...
def atualizar_obra(request, id):
    ...
    if request.method == 'PUT':
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"Mensagem": "Obra atualizada com sucesso"})
...
def atualizar_personagem(request, id):
    try:
        ...
        return Response({"Mensagem": "Personagem não encontrada"}, status=status.HTTP_404_NOT_FOUND)
...
def atualizar_ator(request, id):
    ...
    if request.method == 'PUT':
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"Mensagem": "Ator/Dublador atualizado com sucesso"})
        return Response({"Mensagem": "Formato inválido"}, status=status.HTTP_400_BAD_REQUEST)
...
def atualizar_personagemhabilidade(request, id):
    try:
        ...
        return Response({"Mensagem": "Habilidade do personagem não encontrada"}, status=status.HTTP_404_NOT_FOUND)

    if request.method == 'PUT':
        serializer = PersonagemHabilidadesSerializer(personagemhabilidade, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({"Mensagem": "Habilidade do personagem atualizada com sucesso"})
        return Response({"Mensagem": "Formato inválido"}, status=status.HTTP_400_BAD_REQUEST)
...
def atualizar_relacionamento(request, id):
    try:
        ...
        return Response({"Mensagem": "Relacionamento não encontrada"}, status=status.HTTP_404_NOT_FOUND)
    ...
    if request.method == 'PUT':
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"Mensagem": "Relacionamento atualizada com sucesso"})
        return Response({"Mensagem": "Formato inválido"}, status=status.HTTP_400_BAD_REQUEST)
